...
variables = ['n_hijos', 'region', 'sexo', 'etnia', 'condicion_empleo']
for i in variables:
    datos = datos[~datos[i].isna()]

# Se le da etiquetas más comprensibles
datos["region"] = datos["region"].apply(lambda x: "Costa" if x == 1 else "Sierra" if x == 2 else "Oriente")

# Transformación de variables
variables_categoricas = ['region', 'sexo', 'etnia', 'condicion_empleo']
...
X = datos_dummies[['n_hijos', 'condicion_empleo_Empleada', 'condicion_empleo_Inactiva', 'region_Sierra', 'etnia_Indígena', 'etnia_Mestizo/Blanco', 'etnia_Montubia/o']]
